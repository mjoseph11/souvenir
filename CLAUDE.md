...
Bensalem, PA. It is published as a single HTML file (`index.html`), **generated by `scripts/build.js`**, designed to be printed as a PDF or viewed online.
...
## Build System (since 2026-09-14) — do NOT edit index.html by hand

| Path | What it is |
|------|------------|
| `data/outline.json` | Section and page order, following Manoj's "SGMOC Content Outline.docx" (Drive `1pRHU_HaNkMb-gZDuq5EzBOSb1pZnUMlm`) |
| `data/members.json` | Member & memorial pages as content blocks (photos, text, names, memorial, half, side…). `approved: true` removes the "Proof copy" tag once the family has proofread |
| `data/sponsors.json` | Sponsor ads, spread evenly through the member section |
| `src/pages/*.html` | Hand-built pages (messages, history, ministries, cover, back cover); `src/pages/members/` holds the 7 April member pages |
| `src/styles.css`, `src/members.css` | Styles (members.css = generated-page templates) |
| `images/members/<slug>/` | Member photos, resized to ≤2400px |
| `scripts/build.js` | Builds index.html: page numbers, PAGE comments, A–Z members, Index pages |
| `scripts/check-verified.js` | Verified-facts guard (CI). Facts can be scoped to a page label |
| `scripts/check-layout.js` | Headless-Chrome check for overflow, text under footer, broken/squeezed images |

Run after any change: `node scripts/build.js && node scripts/check-verified.js && node scripts/check-layout.js`

Book structure (126 pages as of 2026-09-14): Front (messages, leadership, clergy, past clergy) → History → Ministries → Church Activities → Members & Memorials A–Z (75 families, ads between) → Index → Acknowledgements → Church family photo → Back cover. Unwritten outline items are dashed "To be completed" placeholder pages. Member content came from the Drive **Design** and **Writing/Editing** folders; see memory for folder IDs.

## Previous page structure (April 2026, superseded)
...
1. Edit `data/*.json` or `src/`, then run `node scripts/build.js && node scripts/check-verified.js && node scripts/check-layout.js` (never edit `index.html` directly — it is regenerated)
